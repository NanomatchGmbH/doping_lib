...
def main():
    ...
    efficiency_path = '../../results/0/corrected_measured_efficiency.csv'
    ...
    df_efficiency.rename(columns={'efficiency_updated': 'experiment_absolute'}, inplace=True)  # abs

    # Merge the DataFrames on the normalized material field
    df_merged = pd.merge(df_efficiency[['material_normalized', 'experiment_absolute']],
                         df_simulation[['material_normalized', 'simulation', 'IP', 'EA']],
                         on='material_normalized',
                         how='outer')

    # Save the merged DataFrame to a new CSV file
    output_path = 'merged_ionization_data_corrected_measurements_abs.csv'
