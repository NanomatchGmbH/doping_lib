...
plot_main = sns.lineplot(x='IP - EA [eV]', y='sigma [S/cm]', data=data, marker=None, label='kMC simulations (5 mol%)',
                         color='black')
...
# Plot extra data
for name, group in extra_data.groupby('name'):
    plt.scatter(group['IP - EA (eV)'], group['Conductivity [S/cm]'], color=color_map[name], label=name, marker='o')
    plt.scatter(group['exper. IP - EA (eV)'], group['Conductivity [S/cm]'], color=color_map[name], marker='x')
...
# Add legend
plt.legend(title='conductivity at: 1..10 mol%')

# Show the plot
# plt.show()
plt.savefig("results/parametric_conductivity/ip_ea_vs_conductivity.png", dpi=600)
plt.close()